...
namespace SandHook {
...
#ifdef __i386__
            *(intptr_t*)((intptr_t)code + OFFSET_JUMP_ADDR_TARGET) = (intptr_t)target - ((intptr_t)code + SIZE_DIRECT_JUMP_TRAMPOLINE);
            flushCache((Size)code + OFFSET_JUMP_ADDR_TARGET, BYTE_POINT);
#else
    ...
    public:
        ...
        void setOriginMethod(Code originMethod, Size sizeNeedBackup) {
            codeCopy(reinterpret_cast<Code>(&originMethod), OFFSET_INLINE_ORIGIN_ART_METHOD, BYTE_POINT);
#if defined(__i386__) || defined(__x86_64__)
            void* codeAfterHook = getEntryCode(originMethod) + sizeNeedBackup;
            codeCopy(reinterpret_cast<Code>(&codeAfterHook), OFFSET_INLINE_ADDR_ORIGIN_CODE_AFTER_HOOK, BYTE_POINT);
#else
